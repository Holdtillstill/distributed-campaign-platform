...
export function RedesignApp({
  ...
  const handleNavigate = (key: string) => {
    ...
    if (routeSyncMode === "company") {
      ...
      if (nextScreen === "new-campaign") onCompanyPage?.("campaigns", { campaignSubpage: "create", path: "/app/campaigns/new" });
      if (nextScreen === "monitor") onCompanyPage?.("campaigns", { campaignSubpage: "monitor", path: "/app/monitor" });
  ...
  const handleModeChange = (m: "company" | "admin") => {
    setMode(m);
    setScreen(m === "admin" ? "admin-dashboard" : "dashboard");
  };

  return (
    <div className={`campaignos-redesign ${theme} flex h-screen w-full overflow-hidden bg-background`} style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <Sidebar
        active={screen}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          {mode === "company" && screen === "new-campaign" && (
            <NewCampaign onBack={() => setScreen("campaigns")} />
          )}
          {screen === "monitor" && (
            <BroadcastMonitor />
          )}
